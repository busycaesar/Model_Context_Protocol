...
server.tool("fetch-collections", "Get all the collections stored in the firebase", async (_extra) => {
    const collections = await firebase.fetchCollection();
    return {
        content: [{ type: "text", text: JSON.stringify(collections) }],
    };
});
server.tool("add-new-document", "Add new document into the existing or new collection. If the collection does not exists, the function creates it automatically.", { collectionName: z.string(), document: z.any() }, async ({ collectionName, document }) => {
...
server.tool("get-documents", "Get all the documents from the existing collections in the firebase", { collectionName: z.string() }, async ({ collectionName }) => {
